export default async (req) => {
  ...
  if (!normalizedEmail.endsWith('@gmail.com')) {
    return new Response(JSON.stringify({ error: 'Email debe ser @gmail.com' }), { status: 400, headers });
  }
  ...
  if (!apellido || apellido.toString().trim().length < 2) {
    return new Response(JSON.stringify({ error: 'Apellido inválido' }), { status: 400, headers });
  }
  const textoTrimmed = (texto || '').toString().trim();
  if (textoTrimmed.length < 20 || textoTrimmed.length > 500) {
    return new Response(JSON.stringify({ error: 'Reseña debe tener entre 20 y 500 caracteres' }), { status: 400, headers });
  }
  const estrellasNum = Number(estrellas);
  if (!estrellasNum || estrellasNum < 1 || estrellasNum > 5) {
    return new Response(JSON.stringify({ error: 'Valoración inválida' }), { status: 400, headers });
  }

  const SUPABASE_URL = process.env.SUPABASE_URL;
  const SUPABASE_SERVICE_KEY = process.env.SUPABASE_SERVICE_KEY;
  const RESEND_API_KEY = process.env.RESEND_API_KEY;
  const EMAIL_FROM = process.env.EMAIL_FROM || 'IndexFreedom <noreply@indexfreedom.xyz>';

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY || !RESEND_API_KEY) {
    console.error('Missing env vars: SUPABASE_URL, SUPABASE_SERVICE_KEY o RESEND_API_KEY');
  ...
  if (!dbRes.ok) {
    ...
    return new Response(JSON.stringify({ error: 'Error al guardar el código' }), { status: 500, headers });
  }
